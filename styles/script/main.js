            ...
            document.querySelectorAll('.whatsapp-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (product) {
                        ...
                        const message = `Hello ProsieFashions! I'm interested in the ${product.name}${colorName ? ' in ' + colorName : ''}. Can you tell me more about this product?`;
                        
                        // Encode message for URL
                        const encodedMessage = encodeURIComponent(message);
                        
                        // Open WhatsApp (replace phone number with actual business number)
                        window.open(`https://wa.me/256750366195?text=${encodedMessage}`, '_blank');
                    }
                });
            });
        }
